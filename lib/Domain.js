...
function Domain(){
    ...
    // look function use it.
    this._rp = {};
    this._es = {};
    this._auto_remove_es = function(){
       setTimeout(function(){
          var t  =  Date.now();
          for(var k in self._es){
            var e = self._es[k];
            if(t-e.time > 1000*20){
                self.off(k,e.handle);
                delete self._es[k];
...
Domain.prototype = {
    ...
    off:function(eventname,handle){
        this._eventBus.removeListener(eventname,handle);
    },
    ...
    look:function(en,t){
        ...
        var self = this;
        if(self._rp[en] && t < self._rp[en].time){
            return self._rp[en].data;
        }else if(self._es[en]){
            self._es[en].time = Date.now();
            if(self._rp[en] && (Date.now - self._rp[en].time) > 20*1000){
               delete self._rp[en];
            }
        }else{
            self._es[en] = {
                handle:function(event){
                   ...
                   var t = Date.now();
                   self._rp[en].data = event.data;
                   self._rp[en].time = t;
                },
                time:Date.now()
            }
            self.on(en,self._es[en].handle);
        }
        ...
        function loop(){
            setTimeout(function(){
                ...
                if(esk.length === 0);
                    ...
                    jq.post(this._url+"/on",{events:JSON.stringify(self._events)},function(rs){
